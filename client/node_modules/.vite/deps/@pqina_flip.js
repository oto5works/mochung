...
var tick_core_module_default = typeof window !== "undefined" ? function() {
  ...
  var toTimezoneOffset = function toTimezoneOffset2(ISO8601Timezone) {
    var current = new Date().getTimezoneOffset() * 6e4;
  ...
  var toGradient = function toGradient2(string2) {
    ...
    var colors = string2.substr(type2.length).match(/(?:transparent|rgb\(.*?\)|hsl\(.*?\)|hsla\(.*?\)|rgba\(.*?\)|[a-z]+|#[abcdefABCDEF\d]+)\s?(?:[\d]{1,3}%?)?/g).map(toGradientColor);
    return {
      type: type2,
      colors
    };
